...
class ImageCController extends BaseController
{
    public function getResponse($template, $filename,Request $request)
    {
        switch (strtolower($template)) {
            ...
            case 'custom':
                return $this->getCustom($filename,$request);
    ...
    public function getCustom($filename,$request)
    {
        $s = $request->size;
        $path = $this->getImagePath($filename);
        $size = explode('x', $s);

        $content = Image::cache(function($image) use($size, $path){
            return $image->make($path)->fit($size[0], $size[1]);
        }, 10, true);
